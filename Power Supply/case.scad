...
module case_power(l=105,w=165,h=50,thickness=2,armatures=ARMATURES,r=8) {
    ...
    difference() {
        ...
        //cut out inside
        translate([0,0,0]) cube([l,w,h+thickness], center=true);
        ...
        translate([-l/2+_border_l-VENT_MARGIN,-w/2+_border_w-VENT_MARGIN,h/2+thickness-1]) 
            _vent(rows=VENT_ROWS,cols=VENT_COLS,size=VENT_SIZE,margin=VENT_MARGIN,thickness=THICKNESS+2);
        ...
        //DC Jacks
        rotate([0,90,0]) translate([h/2-20,w/2-25,-l/2]) DC_jack_hole();
        rotate([0,90,0]) translate([h/2-35,w/2-25,-l/2]) DC_jack_hole();
    }
    _screw_holes(l=l,w=w,h=h,thickness=thickness); 

    if( armatures ) {
        //DC Jacks
        rotate([0,90,0]) translate([h/2-20,w/2-25,-l/2]) DC_jack();
        rotate([0,90,0]) translate([h/2-35,w/2-25,-l/2]) DC_jack();
    }
}
