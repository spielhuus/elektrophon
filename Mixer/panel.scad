    ...
    translate([HP*HP_SIZE/2, 15, 0]) {
        difference() {
            group() {
                ...
                translate([0,95,0]) cylinder(r=2, h=20, $fn=50);
            }
        cylinder(r=0.5, h=20, $fn=50);
        translate([0,95,0]) cylinder(r=0.5, h=20, $fn=50);
        }
    } 
}
...
module panel_mixer(thickness=THICKNESS, screen=SCREEN, font_size=FONT_SIZE,armatures=ARMATURES) {
    difference() {
        ...
        translate([TOP_REFLECTOR_X+1, TOP_REFLECTOR_Y,0]) rotate([0,0,270]) silkscreen(text="MIXER", thickness=thickness, screen=screen, font_size=font_size, font="Designer Block");
    ...
    //draw the refrectors
    translate([TOP_REFLECTOR_X-1, TOP_REFLECTOR_Y, 0]) rotate([0,0,0]) reflector(l=15);
    translate([1*RASTER_X-5, 4*RASTER_Y+RASTER_Y/2+RASTER_Y_TOP, 0]) rotate([0,0,90]) reflector(l=10);

    //draw the stand
    translate([-HP*HP_SIZE/2+6,2,0]) stand();
    translate([+HP*HP_SIZE/2-6,2,0]) stand();

    if(ARMATURES) {
        translate([TOP_REFLECTOR_X+1, TOP_REFLECTOR_Y,0]) rotate([0,0,270]) silkscreen_text(text="MIXER", thickness=thickness, screen=screen, font_size=font_size, font="Designer Block");
